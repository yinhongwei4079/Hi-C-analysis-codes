...
diff <- read.table("table2.diff.csv",sep = ",",header = T)
...
dcol <- data.frame(comparison=unique(diff$comparison),col=c('#1C9A35','#1B4E8C','#BB4D0E','#EDB85D','#6595A3'))
diff <- merge(diff,dcol)

ggplot(data=data.frame(dada))+
  geom_rect(data=cod,aes(xmin = chr_length,xmax = 0, ymin = 0.1, ymax = 0.3),
            fill = "black", alpha = 1)+
  geom_rect(aes(xmin = e, xmax = s, ymin = 0, ymax = 0.4,fill = count), 
            size = 0.15,alpha = 1)+
  geom_rect(data = diff,aes(xmin=end,xmax=start,ymin = 0.5 ,ymax = 1),fill=diff$col,
           size = 0.15,alpha = 0.7)+
  #geom_point(aes(x=Position,y=0.5,color=loci))+
  #scale_y_continuous(limits = c(-2,2))+
  #scale_color_continuous(low="white",high="#F46F44")+
  scale_fill_continuous(low="white",high="#7c2102",)+
...
ggsave("Fig2A.genome.point.modify.pdf", last_plot(), "pdf", width = 7,height = 7)


ggplot(data=data.frame(dada))+
  geom_rect(data = diff,aes(xmin=end,xmax=start,ymin = 0.5 ,ymax = 0.8, fill=col),
            size = 0.15,alpha = 1)+
  #geom_point(aes(x=Position,y=0.5,color=loci))+
  #scale_y_continuous(limits = c(-2,2))+
  #scale_color_continuous(low="white",high="#F46F44")+
  scale_fill_manual(values = dcol$col,labels=dcol$comparison)+
  scale_y_continuous(limits = c(0,1))+
  labs(color = 'Frequency') +
  coord_flip() + 
  facet_grid(. ~ chr, scales = "free", space = "free") + 
...
ggsave("Fig2A.genome.point.modify2.pdf", last_plot(), "pdf", width = 9,height = 9)
